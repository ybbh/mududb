...
use crate::data_type::dt_fn_convert::{FnBase, FnDataLen, FnDefault, FnInputJson, FnInputTextual, FnOutputJson, FnOutputTextual, FnReceive, FnSend, FnSendTo, FnTypeLen};
use crate::data_type::dt_fn_param::{FnParam, FnParamDefault};
use crate::data_type::dt_impl::dat_table::{
    get_dt_name, get_fn_convert, get_opt_fn_compare, get_opt_fn_param, is_all_fixed_len,
};
use crate::data_type::dt_kind::DTKind;
...
pub enum DatTypeID {
    ...
    Object = PRIMITIVE_ID_MAX + 2,
}

// Cache the maximum ID for efficient access
const MAX_ID: DatTypeID = DatTypeID::Object;
...
impl DatTypeID {
    ...
    pub fn fn_param_default(&self) -> Option<FnParamDefault> {
        self.opt_fn_param().as_ref().and_then(|param| param.default.clone())
    }
    ...
    pub fn dat_kind(&self) -> DTKind {
        if self.is_primitive_type() {
            ...
            match self {
                ...
                DatTypeID::Object => DTKind::Object,
