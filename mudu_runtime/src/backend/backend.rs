...
impl Backend {
    pub fn sync_serve(cfg: MuduDBCfg) -> RS<()> {
        ...
        Self::register_web_service(&cfg, &service, canceller_waiter.clone(), init_db_notifier.clone())?;
        Self::register_pg_service(&cfg, &service, canceller_waiter.clone(), init_db_waiter.clone())?;

        service.serve()?;
        Ok(())
    }

    pub fn register_web_service(cfg: &MuduDBCfg, service: &Service, canceller: Waiter, wait_init_db: Notifier) -> RS<()> {
        let ls = LocalSet::new();
        let task = WebHandleTask::new(cfg.clone(), "web service task".to_string(), canceller, Some(wait_init_db));
    ...
    fn register_pg_service(
        ...
        let accept_task = AcceptHandleTask::new(canceller.clone(), socket_addr, senders, wait_notify);
        service.register(TaskWrapper::new_async_local(ls, accept_task))?;

        let session_task = SessionHandleTask::new(
            cfg.data_path.clone(),
            receivers,
            canceller.clone(),
        );
